...
function Navbar({}: Props) {
  ...
  return (
    <nav className="">
      <div
        className={`  ${
          isDashboard
            ...
            : "fixed top-0 px-4 left-0 right-0 z-50"
        } `}
      >
        <div
          className={`px-2  ${
            isDashboard
              ? "rounded-none my-2"
              : "rounded-lg md:rounded-full backdrop-blur-sm border my-5 container max-w-[1280px] mx-auto"
          } `}
        >
              ...
              {user ? (
                ...
                >
                  ...
                  <DropdownMenuContent className="bg-gray-900 w-[200px] absolute top-4 right-0">
                    ...
                    <DropdownMenuSeparator />
                    <DropdownMenuLabel className="hover:bg-white/10 transition-all duration-300">
                      <span
                ...
                <Link to={"/login"}>
                  <Button className="h-[50px] w-[100px] rounded-full text-lg bg-yellow-600 hover:bg-yellow-500 text-white">
                    Login
                  </Button>
                </Link>
              )}
                ...
                <div className="flex flex-col items-center gap-4 mb-4 transition duration-300 ">
                  {navItems.map((item) => (
                    <Link
                      ...
                      key={item.name}
                      className="text-white active:text-yellow-500"
                    >
                      {item.name}
                    </Link>
                  ))}
                  {isAuthenticated ? (
                    <Link to={"/dashboard"}>
                      <Button
                        onClick={() => setIsOpen(false)}
                        className="w-fit px-10 bg-yellow-600 hover:bg-yellow-500 text-white "
                      >
                        Dashboard
                      </Button>
                    </Link>
      ...
      <Modal openDialog={openDialog} setOpenDialog={setOpenDialog} />
    </nav>
  );
}
