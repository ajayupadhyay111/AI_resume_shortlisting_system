    ...
    <div className="p-4 overflow-x-auto">
      <div className="flex flex-col md:flex-row justify-between items-start">
        ...
        <div className="flex justify-end gap-3 mb-3 md:flex-row flex-col">
          <div
            className={`border rounded-md flex justify-between items-center group ${
              input?.length > 0 ? "border-yellow-600" : "border-gray-500"
            }`}
          >
            <input
              type="text"
              placeholder="Search"
              className="outline-none border-none pl-3 w-[80%] py-1 rounded-md"
              value={input}
            ...
            <Search
              className={` ${
                input?.length > 0
                  ...
                  : "text-gray-500"
              } w-[20%] h-full px-3 py-1 transition rounded-r-md`}
              onClick={FilterDataBasedOnInput}
            />
          </div>
        ...
        <table className="w-full border-collapse border border-gray-700 text-left text-sm text-gray-300">
          <thead className="bg-gray-800 text-gray-200">
            <tr>
              ...
              <th
                className="border border-gray-700 p-2 cursor-pointer flex items-center justify-between gap-1"
                onClick={sortByScore}
              >
                Score
                {sortOrder === "asc" ? (
            ...
            {filteredData?.map((candidate) => (
              <tr key={candidate._id} className="hover:bg-gray-800">
                ...
                <td className="px-4 py-2 space-x-2">
                  {candidate.status === "Shortlisted" ? (
                    <button
                      onClick={() => handleRejectCandidate(candidate._id, id!)}
                    ...
                    <button
                      onClick={() => handleSubmitCandidate(candidate._id, id!)}
